app.factory('logsService', function(firebaseUrl, $firebaseAuth, $firebase, $location, $firebaseArray, $firebaseObject){
	var ref = new Firebase(firebaseUrl+'/logs');
	var logs = $firebaseArray(ref);
	return {
		setLog: function(user, info, from, where){
			var today = new Date();
			var newlog = {
				'userid': user,
				'date': today.getTime(),
...
app.factory('SessionService', function($cookies){
	...
	return {
		...
		setCookieObject:function(key, val){
...
app.factory('loginService', function(firebaseUrl, $firebaseAuth, $firebase, $location, $firebaseArray, $firebaseObject, SessionService){
	...
	var Auth = {
		...
		getProfile: function(id){
			var ref = new Firebase(firebaseUrl+'/profiles/'+id);
...
app.factory('adminService', function(firebaseUrl, $firebase, $location, $firebaseArray, $firebaseObject){
	var ref = new Firebase(firebaseUrl);
	var adminFunctions = {
...
app.factory('cursoService', function(firebaseUrl, $firebase, $location, $firebaseArray){
	...
	return{
		validateInfoCurse:function(curse){
			var ref = $firebaseArray(new Firebase(firebaseUrl + 'cursos'));
			console.log(ref);
		},
		registerUser:function(curse){
			var ref = $firebaseArray(new Firebase(firebaseUrl + 'cursos'));
			console.log(ref);
		}
